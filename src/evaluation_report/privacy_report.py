...
class PrivacyReport :
    ...
    def __init__(self, evaluation_dict, data_test = None, qid_columns = None, non_quid_columns = None, dissimilarities = [SimilarityType.DISSIMILARITY], loaded = None) :
        
        if (evaluation_dict != None) : 
            self._evaluation_dict = evaluation_dict
            self._uni_evaluators = {}
            self._privacy_evaluator = {}
            self._dissimilarities = dissimilarities
            for name in evaluation_dict :
                self._uni_evaluators[name] = UnivariateEvaluator(self._evaluation_dict[name][0], 
                                                                self._evaluation_dict[name][1])
                self._privacy_evaluator[name] = PrivacyEvaluator(self._evaluation_dict[name][0], 
                                                                self._evaluation_dict[name][1], 
                                                                data_test, 
                                                                qid_columns,
                                                                non_quid_columns)
            ...
            self._privacy_evaluator = loaded['privacy_evaluator']
            self.results_synthetic = loaded['results_synthetic']
            self.results_real = loaded['results_real']
            self.gower = loaded['gower']
            self.dissimilarity = loaded['dissimilarity']
            self.share_dissimilarity = loaded['share_dissimilarity'] if 'share_dissimilarity' in loaded.keys() else {}
            self.dissimilarity_idx = loaded['dissimilarity_idx']
            self.dissimilarity_idx_test = loaded['dissimilarity_idx_test'] if 'dissimilarity_idx_test' in loaded.keys() else {}
            self.dissimilarity_categorical = loaded['dissimilarity_categorical']
            self.dissimilarity_idx_categorical = loaded['dissimilarity_idx_categorical']
            self.dissimilarity_numerical = loaded['dissimilarity_numerical']
            self.dissimilarity_idx_numerical = loaded['dissimilarity_idx_numerical']
            self.dissimilarity_mean_imbalanced = loaded['dissimilarity_mean_imbalanced']
            self.dissimilarity_idx_mean_imbalanced = loaded['dissimilarity_idx_mean_imbalanced']
            self.dissimilarity_preprocess = loaded['dissimilarity_preprocess'] if 'dissimilarity_preprocess' in loaded.keys() else {}
            self.dissimilarity_idx_preprocess = loaded['dissimilarity_idx_preprocess'] if 'dissimilarity_idx_preprocess' in loaded.keys() else {}
            self.real_dissimilarity = loaded['real_dissimilarity'] if 'real_dissimilarity' in loaded.keys() else {}
            self.epsilon_dissimilarity = loaded['epsilon_dissimilarity'] if 'epsilon_dissimilarity' in loaded.keys() else {}
            self.epsilon_dissimilarity_idx = loaded['epsilon_dissimilarity_idx'] if 'epsilon_dissimilarity_idx' in loaded.keys() else {}
            self._dissimilarities = loaded['dissimilarities'] if 'dissimilarities' in loaded.keys() else {}
    ...
    def build_report(self, no_gower = True) :
        ...
        for j_acc, j in zip(self._evaluation_dict, range(len(self._evaluation_dict))) :
            ...
            results_synthetic = self._privacy_evaluator[j_acc].evaluate_attribute_synthetic_prediction()
            results_real = self._privacy_evaluator[j_acc].evaluate_attribute_real_prediction()
            ...
            self.results_synthetic[j_acc] = results_synthetic
            self.results_real[j_acc] = results_real
